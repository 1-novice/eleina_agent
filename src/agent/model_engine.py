...
class ModelEngine:
    def __init__(self, model_type: str = None):
        self.models = {}
        self.token_usage = {}
        self.current_model = model_type or settings.model_type
    
    def initialize_models(self):
        """初始化模型"""
        try:
            if self.current_model == "qwen_plus":
                print("使用阿里千问 qwen-plus 模型")
                self._initialize_qwen_plus()
            else:
                self._initialize_local_api()
    ...
    def _initialize_local_api(self):
        """初始化本地API模型"""
        if has_httpx:
            ...
            print(f"使用本地API模型（使用urllib）")
    
    def _initialize_qwen_plus(self):
        """初始化Qwen Plus模型"""
        if has_httpx:
            self.models["qwen_plus"] = {
                "client": httpx.Client(base_url=settings.qwen_plus_api_url, timeout=60.0),
                "api_key": settings.qianwen_api_key,
                "model": settings.qwen_plus_model
            }
            print(f"Qwen Plus模型初始化成功，URL: {settings.qwen_plus_api_url}，模型: {settings.qwen_plus_model}")
        else:
            self.models["qwen_plus"] = {
                "client": "urllib",
                "api_key": settings.qianwen_api_key,
            ...
            print(f"Qwen Plus模型初始化成功（使用urllib），URL: {settings.qwen_plus_api_url}")
    
    def switch_model(self, model_type: str):
        """切换模型"""
        self.current_model = model_type
        if model_type not in self.models:
            if model_type == "qwen_plus":
                self._initialize_qwen_plus()
            else:
                self._initialize_local_api()
    ...
    def generate(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model_name = request.get("model", self.current_model)
        ...
        try:
            ...
            if model_name == "qwen_plus":
                if stream:
                    return self._generate_qwen_plus_stream(messages, user_id, add_system_prompt)
                else:
                    return self._generate_qwen_plus(messages, stream, user_id, add_system_prompt)
            else:
                if stream:
    ...
    def generate_stream(self, request: Dict[str, Any]) -> Generator[str, None, None]:
        ...
        try:
            ...
            if model_name == "qwen_plus":
                yield from self._stream_qwen_plus(messages, user_id, add_system_prompt)
            else:
    ...
    def _generate_qwen_plus(self, messages: List[Dict[str, str]], stream: bool, user_id: str, add_system_prompt: bool = True) -> Dict[str, Any]:
        """使用Qwen Plus生成响应"""
        import time
        start_time = time.time()
        try:
            model_info = self.models["qwen_plus"]
            client = model_info.get("client")
            api_key = model_info.get("api_key")
            model_name = model_info.get("model")
            
            if not client or not api_key:
                raise ValueError("Qwen Plus客户端或API密钥未初始化")
            ...
            print(f"[Qwen Plus调用] 用户: {user_id}, 消息数: {len(messages)}, 模型: {model_name}")
            
            if has_httpx and isinstance(client, httpx.Client):
                print(f"[Qwen Plus调用] 开始发送HTTP请求...")
                response = client.post("", json=api_request, headers=headers, follow_redirects=True)
                response.raise_for_status()
                api_response = response.json()
            else:
                import urllib.request
                url = settings.qwen_plus_api_url
            ...
            print(f"[Qwen Plus调用] 请求完成，耗时: {elapsed_time:.2f}秒")
    ...
    def _generate_qwen_plus_stream(self, messages: List[Dict[str, str]], user_id: str, add_system_prompt: bool = True) -> Dict[str, Any]:
        """使用Qwen Plus流式生成响应"""
        try:
            model_info = self.models["qwen_plus"]
            client = model_info.get("client")
            ...
            if not client or not api_key or not has_httpx:
            ...
            print(f"[Qwen Plus调用] 流式输出 - 用户: {user_id}, 消息数: {len(messages)}, 模型: {model_name}")
            
            def stream_generator():
                with client.stream("POST", "", json=api_request, headers=headers, follow_redirects=True, timeout=httpx.Timeout(120.0)) as response:
                    response.raise_for_status()
                    buffer = ""
    ...
    def _stream_qwen_plus(self, messages: List[Dict[str, str]], user_id: str, add_system_prompt: bool = True) -> Generator[str, None, None]:
        """使用Qwen Plus流式生成响应（直接返回生成器）"""
        try:
            model_info = self.models["qwen_plus"]
            client = model_info.get("client")
            api_key = model_info.get("api_key")
            ...
            with client.stream("POST", "", json=api_request, headers=headers, follow_redirects=True) as response:
                response.raise_for_status()
                buffer = ""
